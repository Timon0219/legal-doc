...
let sdk = new MkdSDK();
const AttorneyClientUpdateNdaListPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (document) {
      ...
      try {
        ...
        console.log(fileResult.attachments[0].id);
        if (fileResult.success) {
          const dataList = {
            id: id,
            nda: fileResult.attachments[0]
              ? fileResult.attachments[0].id
              : null,
            // playbook2: fileResult.attachments[1]
            //   ? fileResult.attachments[1].id
            //   : null,
            // playbook3: fileResult.attachments[2]
            //   ? fileResult.attachments[2].id
            //   : null,
          };
          console.log(dataList);
          sdk.setTable("project");
          const result = await sdk.callRestAPI(dataList, "PUT");
          if (!result.error) {
            setLoading(false);
            showToast(globalDispatch, "NDA updated");
            navigate(-1);
          }
        }
      } catch (error) {
  ...
  return (
    <form className="p-5 bg-white shadow rounded mb-10" onSubmit={handleSubmit}>
      <h4 className="text-2xl font-medium">Update NDA</h4>
...
export default AttorneyClientUpdateNdaListPage;
