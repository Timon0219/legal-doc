...
const LawFirmExecutedContractsListPage = () => {
            ...
            data
              ? `${
                  data.user_id
                    ...
                    : "1"
                } AND ${`forkfirm_contract.status=1`}  AND ${
                  data.project_name
                  ...
                  data.assigned_attorney
                    ? `attorney.first_name LIKE '%${data.assigned_attorney}%'`
                    : "1"
                }  AND ${
                  data.assigned_attorney
                    ? `attorney.last_name LIKE '%${data.assigned_attorney}%'`
                    : "1"
                }  AND ${
  ...
  const onSubmit = (data) => {
    ...
    let assigned_attorney = getNonNullValue(data.assigned_attorney);
  ...
  async function openExecutedDocument(id) {
    ...
    if (contracts?.executed_doc2_url || contracts?.executed_doc3_url) {
      navigate(`/lawfirm/view_executed_document/${id}`);
    } else {
  ...
  async function openDocument(id) {
    ...
    if (contracts?.doc2_url || contracts?.doc3_url) {
      navigate(`/lawfirm/view_document/${id}`);
    } else {
    ...
    <>
      ...
      <div className="overflow-x-auto  p-5 bg-white shadow rounded">
        <div className="mb-3 text-center justify-between w-full flex  ">
          <h4 className="text-2xl font-medium">Executed Contract</h4>
        </div>
              ...
              {data.length > 0 &&
                data.map((row, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        className="text-xs text-[#2735d9] underline duration-100 hover:text-purple-500 capitalize"
                        onClick={() => {
                          navigate(
                            "/lawfirm/details_counter_party_email/" + row.id,
                            {
                              state: row,
                            }
                          );
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className=" flex flex-col items-start gap-3">
                        ...
                        {/* <Link
                          to={`/lawfirm/view_executed_document/${row.id}`}
                          className="text-xs text-[#4a5fe6] underline duration-100 hover:text-purple-500 capitalize "
                        >
                        ...
                        <Link
                          to={`/lawfirm/edit_document/${row.id}`}
                          className="text-xs text-[#4a5fe6] underline duration-100 hover:text-purple-500 capitalize "
                        >
                          Edit
                        </Link>
...
export default LawFirmExecutedContractsListPage;
