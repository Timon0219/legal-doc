...
const EditAttorneyStripeOrderPage = () => {
  const { dispatch } = React.useContext(AuthContext);
  const schema = yup
    .object({

      user_id: yup.number().required().positive().integer(),
      price_id: yup.number().required().positive().integer(),
...
const [user_id, setUserId] = useState(0);const [price_id, setPriceId] = useState(0);const [stripe_id, setStripeId] = useState('');const [object, setObject] = useState('');
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!result.error) {
        ...
        navigate("/attorney/stripe_order");
      } else {
  ...
  return (
    <div className=" shadow-md rounded   mx-auto p-5">
      <h4 className="text-2xl font-medium">Edit StripeOrder</h4>
...
export default EditAttorneyStripeOrderPage;
