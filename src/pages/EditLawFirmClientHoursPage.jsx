...
const EditLawFirmClientHoursPage = () => {
    ...
    (async function () {
      try {
        ...
        if (!result.error) {
          ...
          setValue("date", dateHandle(result.model.date));
  ...
  React.useEffect(() => {
    if (clients.length > 0 && client_id) {
      const oldClient = clients.filter((val) => val.value === client_id)[0];
      setClient(oldClient);
    }
  }, [client_id, clients]);
  ...
  return (
    <div className=" shadow-md rounded   mx-auto p-5">
      <h4 className="text-2xl font-medium">Edit ClientHours</h4>
      <form className=" w-full max-w-lg" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="mb-4 ">
          ...
          />
          <p className="text-red-500 text-xs italic">{errors.hour?.message}</p>
        </div>
...
export default EditLawFirmClientHoursPage;
