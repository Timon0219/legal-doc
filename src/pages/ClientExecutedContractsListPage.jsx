...
const ClientExecutedContractsListPage = () => {
  ...
  async function searchData(pageNum, limitNum, data) {
    console.log("RUNING SEarch data");
    setLoading(true);
    try {
      sdk.setTable("contract"); //;
      const result = await sdk.callRestAPI(
  ...
  async function openExecutedDocument(id) {
    ...
    if (contracts?.executed_doc2_url || contracts?.executed_doc3_url) {
      navigate(`/client/view_executed_document/${id}`);
    } else {
  ...
  async function openDocument(id) {
    ...
    if (contracts?.doc2_url || contracts?.doc3_url) {
      navigate(`/client/view_document/${id}`);
    } else {
                      ...
                      >
                        <td className="2xl:hidden px-6 py-4 whitespace-nowrap">
                          ...
                          >
                            ...
                            {/* <Link
                            to={`/client/view_executed_document/${row.id}`}
                            className="text-xs text-[#4a5fe6] underline hover:text-purple-500 duration-100 capitalize "
                          >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {row.lawfirm_first_name || row.lawfirm_last_name
                            ? row.lawfirm_first_name + row.lawfirm_last_name
                            : "N/A"}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          {row.counter_party_name ?? "N/A"}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex flex-col items-start gap-3">
                            <button
                              className="text-xs text-[#4a5fe6] underline duration-100 hover:text-purple-500"
                              onClick={() => {
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button
                            ...
                            onClick={() => {
                              navigate(
                                "/client/document_client_tags/" + row.id,
                                {
                                  state: row,
                                }
                              );
...
export default ClientExecutedContractsListPage;
