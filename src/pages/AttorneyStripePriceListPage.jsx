...
const AttorneyStripePriceListPage = () => {
  ...
  const schema = yup.object({
      ...
      usage_limit: yup.number().positive().integer(),
      object: yup.string(),
      amount: yup.number(),
      trial_days: yup.number().positive().integer(),
      type: yup.string(),
      status: yup.string(),
  });
  const {
...
let id = getNonNullValue(data.id);
    ...
    let product_id = getNonNullValue(data.product_id);
    let stripe_id = getNonNullValue(data.stripe_id);
    let is_usage_metered = getNonNullValue(data.is_usage_metered);
    let usage_limit = getNonNullValue(data.usage_limit);
    let object = getNonNullValue(data.object);
    let amount = getNonNullValue(data.amount);
    let trial_days = getNonNullValue(data.trial_days);
    ...
    let filter = {
      id,
          ...
          stripe_id: stripe_id,
          is_usage_metered: is_usage_metered,
  ...
  return (
    <>
      ...
      >
        <h4 className="text-2xl font-medium">StripePrice Search</h4>
      ...
      <div className="overflow-x-auto  p-5 bg-white shadow rounded">
        <div className="mb-3 text-center justify-between w-full flex  ">
          <h4 className="text-2xl font-medium">StripePrice</h4>
          <AddButton link={"/attorney/add-stripe_price"} />
        </div>
                        ...
                        return (
                          ...
                          >
                            <button
                              className="text-xs"
                              onClick={() => {
                                navigate("/attorney/edit-stripe_price/" + row.id, {
                                  state: row,
                                });
                              }}
                            >
...
export default AttorneyStripePriceListPage;
