...
const AdminLoginPage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true);
    try {
      const result = await sdk.regularLogin(data.email, data.password, "admin");
      ...
      if (!result.error) {
        let body = encodeURI(
          `${location.origin}/two_fa?role=admin&token=${result.one_time_token}`
        );

        const emailTemplate = await sdk.callRawAPI(
          "/v2/api/forkfirm/public/email/PAGINATE",
          { page: 1, limit: 1, where: [`slug = 'two-fa' `] },
          "POST"
        );

        if (
          Array.isArray(emailTemplate.list) &&
          emailTemplate.list.length > 0
        ) {
