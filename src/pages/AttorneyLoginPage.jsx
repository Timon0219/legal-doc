...
const AttorneyLoginPage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      return;
      // const result = await sdk.login(data.email, data.password, "attorney");
      console.log(result);
      if (!result.error) {
        const body = encodeURI(
          `${location.origin}/two_fa?role=attorney&token=${result.one_time_token}`
        );
        ...
        //   console.log(list[0].attorney_license);
        ...
        //     // navigate(`/two_fa/code?role=attorney&qr_code=${result.qr_code}`);
        //   }
        // }
      } else {
        setLoading(false);
